...
int cod;
char diag[30],temp[100],aux[40];

/*inicia a passagem de informacoes para as variaveis auxiliares*/
...
if(p==NULL)/*testa se a lista esta vazia*/
{
 p = (lista *) malloc(sizeof(lista));/*aloca lista se ela estiver vazia*/
 if(p==NULL)/*testa se conseguiu alocar*/
 printf("\t\t\tNao foi possivel executar a operacao !\n\n");
 else{/*se alocou passa as informacoes das variaveis auxiliares para a lista*/
      strcpy(p->nome,"Nome: "); 
      strcat(p->nome,aux);
      strcat(p->nome,"\n");
      
      strcpy(p->diagnose,"Diagnose: ");
      strcat(p->diagnose,diag);
      strcat(p->diagnose,"\n");
      
      strcpy(p->tratamento,"Tratamento: ");
      strcat(p->tratamento,temp);
      strcat(p->tratamento,"\n");
...
else /* testa se ja existir elementos na lista */
{
 novo=(lista*)malloc(sizeof(lista));/*aloca nova lista*/
 if(novo==NULL)/*testa se conseguiu alocar*/
 printf("\t\tNao foi possivel executar a operacao !\n\n");
 else{/*como caso tenha alocada nova lista*/
     /*passagem das informacoes das variaveis para a nova lista*/
      strcpy(novo->nome,"Nome: "); 
      strcat(novo->nome,aux);
      strcat(novo->nome,"\n");
      
      strcpy(novo->diagnose,"Diagnose: ");
      strcat(novo->diagnose,diag);
      strcat(novo->diagnose,"\n");
      
      strcpy(novo->tratamento,"Tratamento: ");
      strcat(novo->tratamento,temp);
      strcat(novo->tratamento,"\n");
      novo->codigo = cod;
     /*coloca a nova lista na lista ja existente*/
     novo->prox=p;
     novo->ant=NULL;
     p->ant=novo;
     return (novo);/*retorna lista atualizada com o novo elemento*/
     }
}
}
